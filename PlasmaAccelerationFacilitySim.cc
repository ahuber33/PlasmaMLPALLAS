/// PlasmaAccelerationFacilitySim.cc
//// Auteur: Arnaud HUBER for ENL group <huber@lp2ib.in2p3.fr>
//// Copyright: 2024 (C) Projet PALLAS

#include "G4UIterminal.hh"
#include "PlasmaAccelerationFacilitySimSteppingAction.hh"
#include "PlasmaAccelerationFacilitySimPhysics.hh"
...
int main(int argc, char **argv)
{
  if (argc < 2)
  {
    G4Exception("Main", "main0004", FatalException,
                "Insufficient input arguments. Usage: ./PlasmaAccelerationFacilitySim [ROOT file name] [events] [macro] [FileReader ON/OFF] [MT ON/OFF] [threads (if MT ON)]");
    return 1;
  }
  ...
  PlasmaAccelerationFacilitySimGeometryConstruction *GeomCons = new PlasmaAccelerationFacilitySimGeometryConstruction;
  runManager->SetUserInitialization(GeomCons);

  // Initialize physics
  runManager->SetUserInitialization(new PlasmaAccelerationFacilitySimPhysics);

  // Initialize user actions
    runManager->SetUserInitialization(new PlasmaAccelerationFacilitySimActionInitialization(outputFile, TotalNParticles, Ncores, flag_MT, GeomCons));
